...
def draw_bounding_box_on_image_array(current_frame_number,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  is_vehicle_detected, csv_line, update_csv,predicted_speed,predicted_direction= draw_bounding_box_on_image(current_frame_number, image_pil, ymin, xmin,
                                                                         ymax, xmax, color,
                                                                         thickness, display_str_list,
                                                                         use_normalized_coordinates)
  np.copyto(image, np.array(image_pil))
  return is_vehicle_detected, csv_line, update_csv,predicted_speed,predicted_direction
...
def draw_bounding_box_on_image(current_frame_number,
  ...
  predicted_speed = "NA"  # means not available, it is just initialization
  predicted_direction = "NA"  # means not available, it is just initialization

  image_temp = np.array(image)
  detected_vehicle_image = image_temp[int(top):int(bottom), int(left):int(right)]
  sys.path.append('C:/Users/DELL/PycharmProjects')
  sys.path.append('C:/Users/DELL/PycharmProjects/experimentsynergy')
  ROI_POSITION=120
  from experimentsynergy import syn
  if (
      ...
      predicted_direction, predicted_speed, is_vehicle_detected, update_csv = syn.speed(top,
                                                                                                bottom,
                                                                                                right,
                                                                                                left,
  ...
  # instead of above.
  csv_line =  str(predicted_direction) + "," + str(predicted_speed)  # csv line created
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,
  ...
  for box, color in box_to_color_map.items():
    ...
    if (("car" in display_str_list[0]) or ("truck" in display_str_list[0]) or ("bus" in display_str_list[0])) or ("motorcycle" in display_str_list[0]):
        is_vehicle_detected, csv_line_util, update_csv, predicted_speed, predicted_direction = draw_bounding_box_on_image_array(current_frame_number,
                                                                                     image,
                                                                                     ymin,
                                                                                     xmin,
                                                                                     ymax,
  ...
  if (1 in is_vehicle_detected):
      ...
      if (class_name == "boat"):
          class_name = "truck"
      csv_line_util = class_name + "," + csv_line_util
  return counter, csv_line_util,predicted_speed,predicted_direction
